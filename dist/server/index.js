...
const controller = ({ strapi }) => ({
  ...
  async ExportController(ctx) {
    ...
    try {
      const entry_order = await strapi.plugin("strapi-5-plugin-dashboard").service("service").exportOrders({ start, end, status: status == "all" ? null : status });
...
const service = ({ strapi }) => ({
  ...
  async getChart({ start, end }) {
    const dates = [];
    const dates_day = [];
    const startDate = dayjs__default.default(start).startOf("day");
    const endDate = dayjs__default.default(end).endOf("day");
    let sales = 0;
    let countOrders = 0;
        ...
        if (order.order_item.length > 0) {
          for (const orderItem of order.order_item) {
            orders.push({
              ...
              shipped_at: order.shipped_at ? dayjs__default.default(order.shipped_at).format("YYYY-MM-DD HH:mm:ss") : Number(order?.order_status) === 2 ? dayjs__default.default(order?.updatedAt).format("YYYY-MM-DD HH:mm:ss") : "",
              total: order.total,
